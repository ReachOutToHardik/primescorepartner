...
export default function TeamPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-up font-body">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="px-3 py-0.5 bg-[#1B2A72] text-white font-mono-num text-[11px] font-bold uppercase rounded-full">
              10% Cut Active
            </span>
          </div>
      ...
      {/* 5. Clickable Member Full Details Modal / Slide-Over */}
      {selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-fade-in">
          <Card variant="elevated" className="max-w-lg w-full p-6 sm:p-8 space-y-6 rounded-2xl bg-white relative">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-mono-num font-bold uppercase tracking-widest text-slate-400">Team Advisor Profile</span>
                <h3 className="font-display font-bold text-xl text-slate-900 mt-0.5">{selectedMember.name}</h3>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 gap-4 text-xs font-medium text-slate-700 bg-slate-50 p-4 rounded-xl border border-slate-200/80">
              <div>
                <span className="text-slate-400 uppercase font-bold tracking-wider text-[10px] block font-mono-num">Email</span>
                <span className="font-semibold text-slate-900 font-mono-num">{selectedMember.email}</span>
              </div>
              <div>
                <span className="text-slate-400 uppercase font-bold tracking-wider text-[10px] block font-mono-num">Phone</span>
                <span className="font-semibold text-slate-900 font-mono-num">{selectedMember.phone}</span>
              </div>
              <div>
                <span className="text-slate-400 uppercase font-bold tracking-wider text-[10px] block font-mono-num">Profession</span>
                <span className="font-semibold text-slate-900">{selectedMember.profession}</span>
              </div>
              <div>
                <span className="text-slate-400 uppercase font-bold tracking-wider text-[10px] block font-mono-num">Location</span>
                <span className="font-semibold text-slate-900">{selectedMember.city}</span>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="p-3 bg-indigo-50/60 rounded-xl border border-indigo-100">
                <span className="text-[10px] font-bold text-slate-400 uppercase font-mono-num block">Total Cases</span>
                <span className="font-mono-num font-extrabold text-lg text-slate-900">{selectedMember.casesCount}</span>
              </div>
              <div className="p-3 bg-amber-50/60 rounded-xl border border-amber-100">
                <span className="text-[10px] font-bold text-slate-400 uppercase font-mono-num block">Member Earned</span>
                <span className="font-mono-num font-extrabold text-lg text-amber-600">{selectedMember.totalMemberPoints} Pts</span>
              </div>
              <div className="p-3 bg-emerald-50/60 rounded-xl border border-emerald-100">
                <span className="text-[10px] font-bold text-slate-400 uppercase font-mono-num block">Your 10% Cut</span>
                <span className="font-mono-num font-extrabold text-lg text-[#1B2A72]">+{selectedMember.overridePointsEarned} Pts</span>
              </div>
            </div>
            ...
            <div className="pt-2 flex justify-end">
              <Button
                onClick={() => setSelectedMember(null)}
                className="bg-[#1B2A72] text-white px-5 py-2.5 text-xs font-bold rounded-xl"
              >
                Close Profile
              </Button>
            </div>
